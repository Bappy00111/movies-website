...
const CardDetels = ({ onCloseDetle }) => {
  const { cartData, setCartData } = useContext(MovieContext);

  const handleRemove = (e, id) => {
    e.preventDefault();
    const foundData = cartData.filter((item) => item.id !== id);
    setCartData([...foundData]);
  };
  return (
          ...
          <div className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14">
            {cartData.length === 0 ? (
              <p className="text-red-600 text-xl">
                Your Card is Emty Please Added Card?
              </p>
            ) : (
              cartData.map(
                (data) => (
